...
const getExpanded = (filters, value) =>
  Object.keys(filters).reduce((acc, key) => ({ ...acc, [key]: value }), {});
...
function Filters({ isExpanded, className = "", title }) {
  ...
  const [expanded, setExpanded] = useState(() =>
    getExpanded(filters, Object.keys(filters).length === 1)
  );
  const { t } = useTranslation();

  return (
    <div
      className={`flex h-full min-w-[220px] flex-col  overflow-hidden ${className}`}
    >
      <div className="flex items-center justify-between gap-5 border-b-2 border-border pb-2">
        <h3 className="pl-3 text-lg font-semibold text-text-primary">
        ...
        <div className="flex items-center gap-1.5">
          <Button
            ...
            onClick={() =>
              setExpanded((prev) =>
                getExpanded(
                  filters,
                  !Object.keys(prev).every((key) => prev[key])
                )
              )
            }
          >
          ...
          {Object.keys(filters).length > 1 && (
            <ToolTip
              content={`Toggle filter condition (currently ${filterCondition})`}
            >
              <Button
                size="small"
                shape="icon"
                onClick={onChangeFilterCondition}
              >
                {filterCondition === "OR" ? <Link /> : <Unlink />}
              </Button>
            </ToolTip>
          )}
          <Button
      ...
      <div
        className={`flex flex-1 flex-col gap-2 divide-y divide-border overflow-auto ${
          isExpanded ? "pr-3" : ""
        }`}
      >
          ...
          >
            ...
            >
              ...
              <ChevronDown
                className={`transition-transform duration-300 ${
                  expanded[key] ? "rotate-180" : "rotate-0"
                }`}
              />
            </Button>
            {filters[key]?.map(({ value, checked }) => (
              <div
                ...
                className={`space-y-4 overflow-hidden px-3 ${
                  expanded[key] ? "h-auto" : "h-0 pb-0"
                }`}
              >
                <div className="flex items-center gap-3 text-sm">
                  <CheckBox
                    checked={checked}
                    onChange={() => onFilter(key, value)}
                  />
                  ...
                  <span className="count text-xs">
                    {
                      initialData?.filter((o) =>
                        value.value ? value.condition(o) : value === o[key]
                      ).length
                    }
                  </span>
                </div>
